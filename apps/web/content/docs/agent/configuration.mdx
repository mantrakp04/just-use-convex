...
Configure the agent system through environment variables and Alchemy infrastructure-as-code. No `wrangler.toml` or manual Cloudflare config needed -- Alchemy generates `wrangler.json` from `alchemy.run.ts`.
...
The agent is deployed via Alchemy (`packages/agent/alchemy.run.ts`), which manages all Cloudflare resources declaratively.
