...
│   └── SandboxPtyService — openPtyTerminal, writePtyTerminal, resizePtyTerminal, closePtyTerminal, listPtyTerminalSessions
...
override async onChatMessage(onFinish, options): Promise<Response> {
  ...
  // 3. Filter message parts by input modalities
  const messagesForAgent = filterMessageParts(this.messages, this.state.inputModalities);

  // 4. Build RAG context from Vectorize
  const retrievalMessage = await buildRetrievalMessage({ env, memberId, queryText });
...
- `openPtyTerminal` -- streaming callable that opens/connects a PTY session and streams output
